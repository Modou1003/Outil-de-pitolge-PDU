            ...
            <table class="min-w-full divide-y divide-gray-100 text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    <tr v-for="w in building_works" :key="w.id" class="hover:bg-gray-50">
                        <td class="px-4 py-2 font-mono text-xs">{{ w.code }}</td>
                        <td class="px-4 py-2">{{ w.name }}</td>
                        <td class="px-4 py-2 text-xs text-gray-600">{{ w.description || '—' }}</td>
                        <td class="px-4 py-2"><span class="rounded-full px-2 py-0.5 text-[10px] font-medium" :class="statusBadge[w.status]">{{ w.status_label }}</span></td>
                        <td v-if="canManage" class="px-4 py-2 text-right">
                            <div class="flex justify-end gap-1">
                                <button class="rounded p-1 text-gray-500 hover:bg-indigo-50 hover:text-indigo-700" title="Modifier">
                                ...
                                <button class="rounded p-1 text-gray-500 hover:bg-red-50 hover:text-red-700" title="Supprimer">
                    ...
                    <tr v-if="!building_works.length">
                        <td :colspan="canManage ? 5 : 4" class="px-4 py-6 text-center text-sm text-gray-500">Créer le premier ouvrage →</td>
                    </tr>
                </tbody>
            </table>
        </div>
