...
// Pondération des ouvrages (doit totaliser 100 %).
const totalWeight = computed(() => ouvrages.value.reduce((s, w) => s + (Number(w.weight_percentage) || 0), 0));
const weightBalanced = computed(() => Math.abs(totalWeight.value - 100) < 0.05);

const updateWeight = (w, value) => {
    if (!canWrite.value) return;
    const weight = Math.max(0, Math.min(100, Number(value) || 0));
    if (weight === Number(w.weight_percentage)) return;
    router.put(
        route('projects.building-works.update', [props.project.id, w.id]),
        { name: w.name, description: w.description, weight_percentage: weight },
        { preserveScroll: true },
    );
};
...
<template>
    <div class="space-y-4">
        ...
        <div v-if="selectedWorkId === null" class="space-y-4">
            ...
            <div
                v-if="ouvrages.length && !weightBalanced"
                class="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-5 py-2 text-xs text-amber-800"
            >
                ...
                Somme des pondérations des ouvrages : <strong>{{ totalWeight.toFixed(1) }} %</strong> — elle devrait totaliser 100 % pour un avancement pondéré fidèle.
            </div>

            <div v-if="ouvrages.length" class="space-y-2">
                <div
                    v-for="w in ouvrages"
                    :key="w.id"
                    class="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white px-5 py-3 shadow-sm ring-1 ring-gray-200 transition hover:shadow-md"
                >
                    <button type="button" class="flex min-w-0 flex-1 items-center text-left" @click="selectedWorkId = w.id">
                        <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex items-center gap-1" @click.stop>
                            <label class="text-[11px] text-gray-500">Pond.</label>
                            <input
                                v-if="canWrite"
                                type="number" min="0" max="100" step="0.1"
                                :value="w.weight_percentage"
                                class="w-16 rounded-md border-gray-300 py-1 text-right text-xs"
                                @change="updateWeight(w, $event.target.value)"
                            />
                            <span v-else class="text-xs font-medium text-gray-700">{{ Number(w.weight_percentage).toFixed(0) }}</span>
