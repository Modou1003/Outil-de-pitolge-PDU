...
// ⚠️ ROUTE DE DIAGNOSTIC TEMPORAIRE — reproduit la création d'un ouvrage pour
// capturer l'erreur 500. À SUPPRIMER après diagnostic.
// Visite : https://<domaine>/debug-ouvrage?key=pdu-test-2026&project=1
Route::get('/debug-ouvrage', function (\Illuminate\Http\Request $request) {
    if ($request->query('key') !== 'pdu-test-2026') {
        abort(403);
    }

    $projectId = (int) $request->query('project', 1);

    try {
        $project = \App\Models\PduProject::findOrFail($projectId);

        $maxCode = \App\Models\BuildingWork::where('pdu_project_id', $project->id)
            ->get()
            ->map(fn ($w) => (int) preg_replace('/[^0-9]/', '', $w->code))
            ->max() ?? 0;
        $code = 'OUV-' . str_pad($maxCode + 1, 3, '0', STR_PAD_LEFT);

        $work = \App\Models\BuildingWork::create([
            'pdu_project_id' => $project->id,
            'code' => $code,
            'name' => 'DEBUG ' . now()->format('H:i:s'),
            'description' => 'Créé par /debug-ouvrage',
            'status' => 'not_started',
        ]);

        // On teste séparément le service d'alertes (suspect n°1 : envoi mail synchrone).
        $alerteError = null;
        try {
            app(\App\Services\AlerteService::class)->generateForAll();
        } catch (\Throwable $e) {
            $alerteError = [
                'classe' => get_class($e),
        ...
        return response()->json([
            'resultat' => '✅ Ouvrage créé sans erreur SQL.',
            'ouvrage' => ['id' => $work->id, 'code' => $work->code],
            'erreur_service_alertes' => $alerteError ?? '✅ Aucune erreur dans generateForAll()',
        ], 200, [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    } catch (\Throwable $e) {
        return response()->json([
            'resultat' => '❌ ÉCHEC.',
            ...
            'message' => $e->getMessage(),
            'fichier' => $e->getFile() . ':' . $e->getLine(),
            'trace' => collect($e->getTrace())->take(8)->map(fn ($t) => ($t['file'] ?? '?') . ':' . ($t['line'] ?? '?'))->all(),
        ], 500, [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    }
});
